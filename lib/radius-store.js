...
function parseVoucherImport(input) {
  const rawCodes = (Array.isArray(input) ? input : String(input || "").split(/[\s,;]+/))
    .map((code) => String(code).trim().toUpperCase())
    .filter(Boolean);
  const seenCodes = new Set();
  const duplicateInputCodes = [];
  const uniqueCodes = [];
  ...
  const validCodes = uniqueCodes.filter((code) => VOUCHER_CODE_PATTERN.test(code));
  const invalidCodes = uniqueCodes.filter((code) => !VOUCHER_CODE_PATTERN.test(code));
  return {
...
function summarizeBulkAction(action, rows, requestedCodes, batchId) {
  ...
  const items = batchId
    ? rows.map((row) => ({ code: row.code, status: row.status, found: true }))
    : requestedCodes.map((code) => {
        const row = rowsByCode.get(code);
        return row
          ? { code, status: row.status, found: true }
          : { code, status: "missing", found: false };
      });
  return {
    action,
...
export async function importVoucherList({ name, duration, pfsenseId, codes }) {
  const parsed = parseVoucherImport(codes);
  if (parsed.rawCount === 0) {
    throw Object.assign(new Error("Aucun code voucher a importer"), { status: 400 });
  }
  if (parsed.validCodes.length > MAX_BATCH_QUANTITY) {
    throw Object.assign(new Error(`Un import ne peut pas depasser ${MAX_BATCH_QUANTITY} vouchers valides`), { status: 400 });
  }

  const durationMinutes = Math.max(1, Number(duration || 1440));
  const nasId = pfsenseId && pfsenseId !== "all" ? Number(pfsenseId) : null;
  const fallbackName = `IMPORT_${new Date().toISOString().replace(/[-:]/g, "").slice(0, 13)}`;
  const batchName = ((name && name.trim()) || fallbackName).toUpperCase();
  let existingCodes = new Set();
  let importedCodes = [];
  let batchId = null;

  await transaction(async (connection) => {
    existingCodes = await findExistingVoucherCodes(connection, parsed.validCodes);
    importedCodes = parsed.validCodes.filter((code) => !existingCodes.has(code));
    if (importedCodes.length === 0) return;

    const [existingBatch] = await connection.execute("SELECT id FROM voucher_batches WHERE UPPER(name) = :name LIMIT 1", { name: batchName });
    if (existingBatch.length > 0) {
      throw Object.assign(new Error(`Le lot ${batchName} existe deja`), { status: 409 });
    }
    ...
    const [result] = await connection.execute(
      `INSERT INTO voucher_batches (name, quantity, duration_minutes, nas_id)
       VALUES (:name, :quantity, :duration, :nasId)`,
      { name: batchName, quantity: importedCodes.length, duration: durationMinutes, nasId }
    );
    batchId = result.insertId;
    await insertVoucherBatchRows(connection, batchId, importedCodes, durationMinutes, nasId);
  });

  const importedSet = new Set(importedCodes);
  const importResult = {
    batchId: batchId ? String(batchId) : null,
    batchName: batchId ? batchName : null,
    requested: parsed.rawCount,
    unique: parsed.uniqueCodes.length,
    imported: importedCodes.length,
    duplicateInput: parsed.duplicateInputCodes.length,
    existing: existingCodes.size,
    invalid: parsed.invalidCodes.length,
    items: [
      ...parsed.uniqueCodes.map((code) => {
        if (!VOUCHER_CODE_PATTERN.test(code)) return { code, status: "invalid" };
        if (existingCodes.has(code)) return { code, status: "existing" };
        if (importedSet.has(code)) return { code, status: "imported" };
        return { code, status: "invalid" };
      }),
      ...parsed.duplicateInputCodes.map((code) => ({ code, status: "duplicate_input" }))
    ]
  };

  return { ...(await getSnapshot()), importResult };
}
